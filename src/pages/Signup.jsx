...
const Signup = () => {
  ...
  const handleSignUp = (e) => {
    ...
    if (!/^(.{6,})$/.test(password)) {
      ...
      return toast.error("Password must be at least a capital letter", {
        id: toastId,
      });
    } else if (!/.*[!@#$%^&*()_+{}[\]:;<>,.?~\\-].*/.test(password)) {
      return toast.error("Password must be at least a special character", {
        id: toastId,
      });
    } else if (!/.*\d.*/.test(password)) {
      return toast.error("Password must be at least a numeric character", {
        id: toastId,
      });
    }
          ...
          <div className="flex flex-col mt-5 relative">
            ...
            {passwordType ? (
              <AiFillEyeInvisible
                ...
                className="absolute bottom-3 text-gray-500 right-4 text-xl"
              ></AiFillEyeInvisible>
            ) : (
              <AiFillEye
                onClick={() => setPasswordType(!passwordType)}
